...
+ (BOOL)isExsitsWithUid:(NSInteger)uid
{
    ...
    sqlite3 *db = nil ;
    const char *sql = [[NSString stringWithFormat:@"select * from tab_usersettings where us_uid = %d", uid] UTF8String];
...
+ (BOOL)addWithModel:(ModelUserSettings *)model
{
    ...
    const char *sql = [[NSString stringWithFormat:@"insert into tab_usersettings "
                        ...
                        "(%d, %lf, %d, %d, %d);", model.uid, model.updateTimeInterval, model.shouldSyncHome, model.shouldSyncFavorite, model.shouldSyncHistory]
                       UTF8String];
    char *msg=nil;
    do {
        if (SQLITE_OK!=sqlite3_open(filename, &db))
...
+ (BOOL)updateModel:(ModelUserSettings *)model withUid:(NSInteger)uid
{
    ...
    const char *sql = [[NSString stringWithFormat:@"update tab_usersettings "
                        ...
                        "us_shouldSyncFavorite = %d, us_shouldSyncHistory = %d where us_uid = %d;",
...
+ (BOOL)deleteWithUid:(NSInteger)uid
{
    ...
    const char *filename = [GetDBPathWithName(DB_NAME) UTF8String];
    sqlite3 *db;
    const char *sql = [[NSString stringWithFormat:@"delete from tab_usersettings where us_uid=%d", uid]
                       UTF8String];
    char *msg=nil;
    do {
        if (SQLITE_OK!=sqlite3_open(filename, &db))
